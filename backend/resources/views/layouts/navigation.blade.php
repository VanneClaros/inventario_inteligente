...
.navbar-logo svg { fill: #6ee7b7 !important; width: 28px; height: 28px; }
.navbar-brand { font-size: 0.95rem; font-weight: 700; color: #fff; letter-spacing: -0.02em; }
.navbar-brand span { color: #6ee7b7; }
.navbar-links { display: flex; align-items: center; gap: 0.25rem; flex: 1; }
.nav-link {
    display: flex; align-items: center; gap: 0.4rem;
    padding: 0.4rem 0.85rem; border-radius: 8px;
    font-size: 0.83rem; font-weight: 500; color: #9ca3af;
    text-decoration: none; transition: all 0.15s; white-space: nowrap;
}
.nav-link:hover { background: #1e2130; color: #e8eaf0; }
.nav-link.active { background: #1e2130; color: #6ee7b7; }
...
.nav-link-ia:hover, .nav-link-ia.active {
    background: linear-gradient(135deg, #c9a84c, #e8c96a);
    color: #111; border-color: #c9a84c;
}
.ia-dot {
...
.nav-divider { width: 1px; height: 20px; background: #2e3347; margin: 0 0.25rem; }
...
.user-avatar {
    ...
    font-size: 0.65rem; font-weight: 700; color: #0f1117; flex-shrink: 0;
}
.user-info { text-align: left; }
.user-name { font-size: 0.8rem; font-weight: 600; color: #e8eaf0; line-height: 1; }
.user-rol { font-size: 0.65rem; color: #6b7280; line-height: 1.3; }
.user-chevron { color: #6b7280; font-size: 0.7rem; transition: transform 0.2s; }
.user-area.open .user-chevron { transform: rotate(180deg); }
.dropdown-menu {
    display: none; position: absolute; right: 0; top: calc(100% + 0.5rem);
    background: #1e2130; border: 1px solid #2e3347; border-radius: 12px;
    min-width: 180px; overflow: hidden; box-shadow: 0 8px 32px rgba(0,0,0,0.4);
}
.user-area.open .dropdown-menu { display: block; }
.dropdown-header { padding: 0.85rem 1rem; border-bottom: 1px solid #2e3347; }
.dropdown-email { font-size: 0.72rem; color: #6b7280; }
.dropdown-link {
    display: flex; align-items: center; gap: 0.5rem;
    padding: 0.65rem 1rem; font-size: 0.82rem; color: #9ca3af;
    text-decoration: none; transition: all 0.15s; cursor: pointer;
    background: none; border: none; width: 100%; font-family: 'DM Sans', sans-serif;
}
.dropdown-link:hover { background: #16213e; color: #e8eaf0; }
.dropdown-link.danger:hover { background: #2a1f1f; color: #fca5a5; }
.dropdown-divider { height: 1px; background: #2e3347; margin: 0.2rem 0; }
.hamburger {
    display: none; background: none; border: 1px solid #2e3347;
    border-radius: 8px; padding: 0.4rem; cursor: pointer; color: #9ca3af;
}
.mobile-menu { display: none; background: #0f1117; border-top: 1px solid #1e2130; padding: 0.75rem 1.5rem 1rem; }
.mobile-menu.open { display: block; }
.mobile-link {
    display: flex; align-items: center; gap: 0.5rem;
    padding: 0.65rem 0.75rem; border-radius: 8px;
    font-size: 0.85rem; color: #9ca3af; text-decoration: none; transition: all 0.15s;
}
.mobile-link:hover { background: #1e2130; color: #e8eaf0; }
.mobile-link-ia {
    display: flex; align-items: center; gap: 0.5rem;
    padding: 0.65rem 0.75rem; border-radius: 8px;
    font-size: 0.85rem; font-weight: 600; color: #c9a84c;
    text-decoration: none; background: #c9a84c11; border: 1px solid #c9a84c33;
    margin-bottom: 0.25rem;
}
.mobile-divider { height: 1px; background: #2e3347; margin: 0.5rem 0; }
.mobile-user { display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; margin-bottom: 0.5rem; }
...
<nav class="navbar" x-data="{ open: false, userOpen: false }">
    <div class="navbar-inner">
        ...
        <div class="navbar-links">
            ...
            @if(Auth::check() && Auth::user()->rol == 'admin')
                <div class="nav-divider"></div>
                <a href="/ia/dashboard" class="nav-link-ia {{ request()->is('ia*') ? 'active' : '' }}">
        ...
        <div class="user-area" :class="{ 'open': userOpen }" @click.outside="userOpen = false">
            ...
            <div class="dropdown-menu">
                ...
                <a href="{{ route('profile.edit') }}" class="dropdown-link">👤 Mi perfil</a>
                <div class="dropdown-divider"></div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="dropdown-link danger">🚪 Cerrar sesión</button>
                </form>
            </div>
        </div>
    ...
    <div class="mobile-menu" :class="{ 'open': open }">
        ...
        @if(Auth::check() && Auth::user()->rol == 'admin')
            <div class="mobile-divider"></div>
            <a href="/ia/dashboard" class="mobile-link-ia">🤖 Panel de IA</a>
        @endif
        <div class="mobile-divider"></div>
